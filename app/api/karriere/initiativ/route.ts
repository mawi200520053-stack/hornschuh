...
export async function POST(req: NextRequest) {
  ...
  const safeBereich = escapeHtml(bereich);
  ...
  const subjectText = `Initiativbewerbung: ${vorname.replace(/[\r\n]/g, " ")} ${nachname.replace(/[\r\n]/g, " ")}${bereich ? ` – ${bereich.replace(/[\r\n]/g, " ")}` : ""}`;
      ...
      html: `
        <div style="font-family:Arial,sans-serif;max-width:600px;color:#1a1a1a">
          <div style="background:#1a1a1a;padding:24px 32px;border-radius:8px 8px 0 0">
            ...
            ${safeBereich ? `<p style="color:rgba(255,255,255,0.6);margin:6px 0 0;font-size:14px">Gewünschter Bereich: ${safeBereich}</p>` : ""}
          </div>
          <div style="padding:32px;border:1px solid #e5e5e5;border-top:none;border-radius:0 0 8px 8px">
            <table style="width:100%;border-collapse:collapse">
              ...
              ${safeBereich ? `<tr><td style="padding:8px 0;color:#666">Bereich</td><td style="padding:8px 0">${safeBereich}</td></tr>` : ""}
